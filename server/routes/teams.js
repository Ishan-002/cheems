      ...
      .then((user) => {
        ...
        Team.findById(ObjectID(req.params.id))
            .then((team) => {
              ...
              // if problem comes in getting the id of the user, then do
              // .findOne() and then get the user
...
router.get(
    ...
    middleware.isTeamParticipant, (req, res) => {
      if (!ObjectID.isValid(req.params.id)) {
        return res.status(400).send();
        // return res.redirect('/');
      }
      ...
      Team.findById(ObjectID(req.params.id))
          ...
          .then((team) => {
            ...
            const username = req.body.username;
            User.findOne({username}).populate('teams').then((user) => {
